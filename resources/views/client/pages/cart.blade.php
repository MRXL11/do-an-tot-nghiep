            ...
            <div class="table-responsive bg-white">
                @if(!Auth::check())
                    ...
                    <div class="p-4 text-center">
                        ...
                        <p>Vui lòng <a href="{{ route('login') }}" class="text-primary">đăng nhập</a> để xem và mua sắm sản phẩm.</p>
                    </div>
                @elseif($cartItems->isEmpty())
                    <!-- Người dùng đã đăng nhập nhưng giỏ hàng trống -->
                    <div class="p-4 text-center">
                        <h5>Chưa có sản phẩm nào trong giỏ hàng</h5>
                        <p><a href="{{ route('home') }}" class="text-primary">Mua sắm ngay</a> để thêm sản phẩm vào giỏ.</p>
                    </div>
                @else
                    ...
                    <table class="table align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr id="row-{{ $item->id }}" class="{{ $item->productVariant->status != 'active' ? 'inactive-item' : '' }}">
                                <td>
                                ...
                                <td>
                                    <img src="{{ asset($item->productVariant->image ?? 'path/to/default.jpg') }}" alt="Ảnh" width="60">
                                </td>
                                <td>
                    ...
                    <div class="bg-white shadow-sm p-3 mt-3 d-flex flex-column flex-md-row justify-content-between align-items-center">
                        ...
                        <div class="text-end">
                            ...
                            <a href="#" class="btn btn-warning text-white btn-checkout">Tiến hành thanh toán</a>
                        </div>
                    </div>
                @endif
            </div>
    ...
    $(document).ready(function() {
        ...
        $('.btn-minus').click(function() {
            ...
            if (product.status !== 'active') {
                Swal.fire({ icon: 'warning', title: 'Sản phẩm ngừng hoạt động', text: 'Không thể thay đổi!', timer: 1500, scrollbarPadding: false });
                return;
            }
            ...
            if (qty >= 1) {
                ...
                Swal.fire({ icon: 'info', title: 'Thông báo', text: 'Số lượng tối thiểu là 1.', timer: 1500, scrollbarPadding: false });
            }
        });
        ...
        $('.btn-plus').click(function() {
            ...
            if (qty <= stock) {
                ...
                Swal.fire({ icon: 'info', title: 'Thông báo', text: `Chỉ còn ${stock} sản phẩm trong kho.`, timer: 1500, scrollbarPadding: false });
            }
        });
                ...
                if (result.isConfirmed) {
                    $.post('{{ route("cart.remove") }}', {
                        ...
                        if (res.success) {
                            ...
                            $('#cart-count').text(res.newCartCount).toggle(res.newCartCount != 0);

                            Swal.fire({ icon: 'success', title: 'Đã xóa!', timer: 1000, scrollbarPadding: false });
                        }
                    });
                }
            });
        ...
        $('.btn-remove-selected').click(function(e) {
            e.preventDefault();
            const selected = $('.item-checkbox:checked').map(function() { return $(this).data('id'); }).get();
            if (selected.length === 0) {
                Swal.fire({ icon: 'info', title: 'Chưa chọn sản phẩm!', timer: 1500, scrollbarPadding: false });
                return;
            }
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.post('{{ route("cart.removeSelected") }}', {
                        ...
                        if (res.success) {
                            ...
                            Swal.fire({ icon: 'success', title: 'Đã xóa các sản phẩm đã chọn!', timer: 1200, scrollbarPadding: false });
                        }
                    });
                }
            });
        ...
        $('.btn-checkout').click(function(e) {
            ...
            if (!canCheckout) {
                Swal.fire({ icon: 'warning', title: 'Không thể thanh toán', text: 'Vui lòng chọn ít nhất 1 sản phẩm để thanh toán.', timer: 2000, scrollbarPadding: false});
                return;
            }
