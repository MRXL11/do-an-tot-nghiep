    ...
    $(document).ready(function() {
        ...
        $(document).on('blur', '.quantity-input', function() {
            ...
            if (typeof stock !== 'undefined' && qty > stock) {
                Swal.fire({ icon: 'info', title: 'Thông báo', text: `Chỉ còn ${stock} sản phẩm trong kho.`, timer: 1500 });
                qty = stock;
            }
        ...
        $(document).on('click', '.btn-minus', function() {
            ...
            if (product.status !== 'active') {
                Swal.fire({ icon: 'warning', title: 'Sản phẩm ngừng hoạt động', text: 'Không thể thay đổi!', timer: 1500 });
                return;
            }
            ...
            if (qty >= 1) {
                ...
                Swal.fire({ icon: 'info', title: 'Thông báo', text: 'Số lượng tối thiểu là 1.', timer: 1500 });
            }
        });
                ...
                if (result.isConfirmed) {
                    $.post('{{ route("cart.remove") }}', {
                        ...
                        if (res.success) {
                            ...
                            Swal.fire({ icon: 'success', title: 'Đã xóa!', timer: 1000 });
                        }
                    });
                }
            });
        ...
        $(document).on('click', '.btn-remove-selected', function(e) {
            ...
            if (selected.length === 0) {
                Swal.fire({ icon: 'info', title: 'Chưa chọn sản phẩm!', timer: 1500 });
                return;
            }
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.post('{{ route("cart.removeSelected") }}', {
                        ...
                        if (res.success) {
                            ...
                            if ($('.item-checkbox').length === 0) location.reload();
                            $('#cart-count').text(res.newCartCount).toggle(res.newCartCount != 0);
                            Swal.fire({ icon: 'success', title: 'Đã xóa các sản phẩm đã chọn!', timer: 1200 });
                        }
                    });
                }
            });
        ...
        $(document).on('click', '.btn-checkout', function(e) {
            ...
            if (!canCheckout) {
                Swal.fire({ icon: 'warning', title: 'Không thể thanh toán', text: 'Vui lòng chọn ít nhất 1 sản phẩm để thanh toán.', timer: 2000 });
                return;
            }
